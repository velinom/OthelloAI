...
public class OthelloSolver {
  ...
  // Reads the first line of std in to parse the desired search depth;
  ...
  // Reads the game board from std in
  ...
  // Returns a list of possible moves for the give player (white or black)
  ...
  static boolean capturesInDir(int[][] board, int row, int row_delta,
                               int col, int col_delta, boolean whiteTurn) {
  ...
  // Evaluation function of the current board state.
  // Simply number of white pieces - number of black pieces
  static float evaluationFunction(int[][] board) {
    int count = 0;
    // loop over all tiles and increment white/black counts
    for (int row = 0; row < NUM_COLUMNS; row++) {
      for (int col = 0; col < NUM_COLUMNS; col++) {
  ...
  static float minimax_value(int board[][], boolean whiteTurn, int searchDepth,
